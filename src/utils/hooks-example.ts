...
class AppStateManager {
  ...
  private setupEventListeners() {
    // 监听服务启动事件
    const unsubscribeServiceStart = useEvent(
      ServiceEvents.SERVICE_STARTED,
      (data) => {
    ...
    // 监听配置变更事件
    const unsubscribeConfigChange = useEvent(
      'config:changed',
      (data) => {
    ...
    // 监听用户添加事件
    const unsubscribeUserAdded = useEvent(
      'user:added',
      (data) => {
